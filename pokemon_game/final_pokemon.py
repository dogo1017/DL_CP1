...
def enter_city(city_name):
    ...
    while True:
        ...
        if selected == "center":
            ...
            if city["gym_leader"]["badge"] not in gym_badges:
...
def gym_battle(gym_leader):
    ...
    # Create gym leader's Pokemon
    leader_pokemon = {
        "species_name": "Leader Pokemon",
        "name": f"{gym_leader['name']}'s Pokemon",
        "level": 15 + (len(gym_badges) * 5),
        "type": [gym_leader["type"]],
        "hp": 60 + (len(gym_badges) * 10),
        "max_hp": 60 + (len(gym_badges) * 10),
        "atk": 12 + (len(gym_badges) * 2),
        "defense": 10 + (len(gym_badges) * 2),
        "speed": 10 + len(gym_badges),
...
def travel_route(route_name, destination):
    ...
    for i in range(route["distance"]):
        ...
        if random.random() < 0.4:  # 40% chance per segment
            ...
            if random.random() < route["trainer_chance"]:
                ...
                if trainer_id not in defeated_trainers:
                    ...
                    trainer_pokemon = {
                        ...
                        "level": 8 + len(gym_badges) * 3,
                        "type": ["normal"],
                        "hp": 40 + len(gym_badges) * 5,
                        "max_hp": 40 + len(gym_badges) * 5,
                        "atk": 10 + len(gym_badges),
                        "defense": 8 + len(gym_badges),
                    ...
                    if result == "win":
                        ...
                        prize_money = 100 + (len(gym_badges) * 50)
                        global player_money
                ...
                level = random.randint(5 + len(gym_badges) * 2, 8 + len(gym_badges) * 2)
